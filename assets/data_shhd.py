...
building_cost_base = 0x172B1C
building_health_base = 0x16B584
building_population_base = 0x16B7A0
unit_health_base = 0x3F8D94
unit_arrow_dmg_base = 0x3F8EAC
unit_xbow_dmg_base = 0x3F8FC4
unit_stone_dmg_base = 0x3F90D8

# 16 bytes from last of one unit to start of next unit
unit_melee_dmg_base = 0x3F91F0
resource_buy_base = 0x1BD250
resource_sell_base = 0x1BD2E0
scenario_pgr_base = 0x3A68E8
scenario_pgr_crowded_base = 0x3A6940
skirmish_pgr_base = 0x3A6998
...
def get_unit_melee_dmg_address(attacker_name, defender_name):
    ...
    if defender_index == -1:
        raise Exception("Invalid unit name")
    return unit_melee_dmg_base + defender_index * 4 + attacker_index * (len(unit_names) + 1) * 4
